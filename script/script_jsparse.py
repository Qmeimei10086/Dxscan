...
js_headers = {'User-Agent': 'Mozilla/5.0'}
class JsParse():
    ...
    def jsparse(self):
        out = []
        try:
            r = requests.get(self.host, headers=js_headers , timeout=TIMEOUT, verify=False)
    ...
    def get_js(self, url):
        if not re.search('http://|https://', url):
            if '//' in url:
                ...
                url = self.host + '/' + url
        r = requests.get(url, headers=js_headers , timeout=TIMEOUT, verify=False)
        regex = (
            # 匹配url
