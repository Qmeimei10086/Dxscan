    ...
    def run(self):
        ...
        try:
            ...
            if r"Active Internet connections" in req.text:
                ...
                postdata = {self.url:"存在垚捷电商平台struts 命令执行漏洞...(高危)\tpayload: "+vulnurl+"\t[Linux]"}
                ...
                postdata = {self.url:"存在垚捷电商平台struts 命令执行漏洞...(高危)\tpayload: "+vulnurl+"\t[Windows]"}
                requests.post('http://localhost:8848/cms', json=postdata)

            elif r"LISTEN" in req.text:
                cprint("[+]可能存在垚捷电商平台struts 命令执行漏洞...(高危)\tpayload: "+vulnurl, "red")
                postdata = {self.url:"可能存在垚捷电商平台struts 命令执行漏洞...(高危)\tpayload: "+vulnurl}
                requests.post('http://localhost:8848/cms', json=postdata)
