    ...
    def run(self):
        ...
        try:
            ...
            if r"Active Internet connections" in req.text:
                ...
                postdata = {self.url:"存在九羽图书馆系统struts 命令执行漏洞...(高危)\tpayload: "+vulnurl+"\t[Linux]"}
                ...
                postdata = {self.url:"存在九羽图书馆系统struts 命令执行漏洞...(高危)\tpayload: "+vulnurl+"\t[Windows]"}
                requests.post('http://localhost:8848/cms', json=postdata)
            elif r"LISTEN" in req.text:
                cprint("[+]可能存在九羽图书馆系统struts 命令执行漏洞...(高危)\tpayload: "+vulnurl, "red")
                postdata = {self.url:"可能存在九羽图书馆系统struts 命令执行漏洞...(高危)\tpayload: "+vulnurl}
                requests.post('http://localhost:8848/cms', json=postdata)
            else:
                cprint("[-]不存在jiuyu_library_struts_exec漏洞", "white", "on_grey")
